 ...
 * @group mod_astra
 */
class mod_astra_category_testcase extends advanced_testcase {
    ...
    public function test_createNew() {
        ...
        $catId = mod_astra_category::createNew((object) $catData);
        
        $this->assertNotEquals(0, $catId);
        $category = mod_astra_category::createFromId($catId);
        ...
        $this->assertEquals(1, $DB->count_records(mod_astra_category::TABLE, array('course' => $this->course->id)));
    }
    ...
    public function test_getCategoriesInCourse() {
        ...
        for ($i = 1; $i <= $numCats; ++$i) {
            $catData = array(
                    'course' => $this->course->id,
                    'status' => ($i == 3 ? mod_astra_category::STATUS_HIDDEN : mod_astra_category::STATUS_READY),
                    'name' => "Test category $i",
                    'pointstopass' => 0,
            );
            $catIds[] = mod_astra_category::createNew((object) $catData);
        }
        
        $anotherCourse = $this->getDataGenerator()->create_course();
        $catIds[] = mod_astra_category::createNew((object) array(
        ...
        $fetchedCats = mod_astra_category::getCategoriesInCourse($this->course->id, false);
        ...
        $fetchedCatsHidden = mod_astra_category::getCategoriesInCourse($this->course->id, true);
        ...
        $fetchedCatsHidden = mod_astra_category::getCategoriesInCourse($anotherCourse->id, true);
    ...
    public function test_updateOrCreate() {
        ...
        $newCatId = mod_astra_category::updateOrCreate((object) $catData); // should create new category
        
        $this->assertEquals(1, $DB->count_records(mod_astra_category::TABLE, array('course' => $this->course->id)));
        $cat = mod_astra_category::createFromId($newCatId);
        $this->assertEquals('Some category', $cat->getName());
        $this->assertEquals(mod_astra_category::STATUS_READY, $cat->getStatus());
        
        // update the category
        $catData['status'] = mod_astra_category::STATUS_HIDDEN;
        $catData['pointstopass'] = 17;
        $catId = mod_astra_category::updateOrCreate((object) $catData);
        
        $this->assertEquals($newCatId, $catId);
        $cat = mod_astra_category::createFromId($catId);
        $this->assertEquals('Some category', $cat->getName());
        $this->assertEquals(mod_astra_category::STATUS_HIDDEN, $cat->getStatus());
    ...
    public function test_delete() {
        ...
        $round2 = new mod_astra_exercise_round($record);
        ...
        $category->delete();
        $this->assertEquals(1, $DB->count_records(mod_astra_category::TABLE, array('course' => $this->course->id)));
        $cats = mod_astra_category::getCategoriesInCourse($this->course->id);
        $this->assertEquals($category2->getId(), current($cats)->getId());
        $this->assertEquals(0, $DB->count_records(mod_astra_learning_object::TABLE, array('categoryid' => $category->getId())));
        // the other category should remain unchanged
        $this->assertEquals(2, $DB->count_records(mod_astra_learning_object::TABLE, array('categoryid' => $category2->getId())));
    }
}