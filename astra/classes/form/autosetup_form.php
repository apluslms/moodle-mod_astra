 ...
 * Stratum2 exercises in a course.
 */
class autosetup_form extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'configurl', get_string('configurl', \mod_stratumtwo_exercise_round::MODNAME));
        $mform->setType('configurl', PARAM_NOTAGS);
        $mform->addHelpButton('configurl', 'configurl', \mod_stratumtwo_exercise_round::MODNAME);
        $mform->addRule('configurl', null, 'required', null, 'client');
        $mform->addRule('configurl', null, 'maxlength', 255, 'client');

        // server API key
        $mform->addElement('text', 'apikey', get_string('apikey', \mod_stratumtwo_exercise_round::MODNAME));
        $mform->setType('apikey', PARAM_NOTAGS);
        $mform->addHelpButton('apikey', 'apikey', \mod_stratumtwo_exercise_round::MODNAME);
        ...
        $mform->addElement('text', 'sectionnum',
                get_string('sectionnum', \mod_stratumtwo_exercise_round::MODNAME));
        $mform->setType('sectionnum', PARAM_INT);
        $mform->addHelpButton('sectionnum', 'sectionnum', \mod_stratumtwo_exercise_round::MODNAME);
        ...
        $this->add_action_buttons(true,
            get_string('apply', \mod_stratumtwo_exercise_round::MODNAME));
    }
}
